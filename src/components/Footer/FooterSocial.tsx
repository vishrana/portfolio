import * as React from 'react'

export default function FooterSocial() {
  return (
    <div className="social d-flex justify-content-center">
      <ul className="icon-list">
        {/* <li><a className="btn" target="_blank" rel="noopener noreferrer" href="https://twitter.com/fabasoad"><i className="fab fa-twitter fa-2x"></i></a></li> */}
        <li><a className="btn" target="_blank" rel="noopener noreferrer" href="https://github.com/vishrana"><i className="fab fa-github fa-2x"></i></a></li>
        <li><a className="btn" target="_blank" rel="noopener noreferrer" href="https://www.linkedin.com/in/ranavish/"><i className="fab fa-linkedin fa-2x"></i></a></li>
        {/* <li><a className="btn" target="_blank" rel="noopener noreferrer" href="https://stackoverflow.com/users/470214/fabasoad"><i className="fab fa-stack-overflow fa-2x"></i></a></li> */}
        {/* <li><a className="btn" target="_blank" rel="noopener noreferrer" href="https://dev.to/fabasoad"><i className="fab fa-dev fa-2x"></i></a></li> */}
      </ul>
    </div>
  )
}
